...
def test_read_capy_nc_wCapyShape():
    wCapy, addedMass, damping, fExcitation = fdo.read_capy_nc(capyTestFile)
    assert len(wCapy) == 28
...
def test_read_capy_nc_fExcitationShape():
    wCapy, addedMass, damping, fExcitation = fdo.read_capy_nc(capyTestFile)
    assert fExcitation.shape == (6, 28)

def test_read_capy_nc_wRangeCheck():
    with pytest.raises(ValueError):
        wDes = np.arange(0.01, 3, 0.01)
        wCapy, addedMass, damping, fExcitation = fdo.read_capy_nc(capyTestFile, wDes=wDes)
...
def test_read_capy_nc_fExRealVals():
    ...
    assert max(abs(refFExReal[:,1] - fExcitation[0,:].real)) < 1e-12

def test_read_capy_nc_fExImagVals():
    wCapy, addedMass, damping, fExcitation = fdo.read_capy_nc(capyTestFile)
    refFExImag = np.loadtxt(f'./ref_data/capytaine_integration/wCapy-fExcitationImag-surge.txt')
    assert max(abs(refFExImag[:,1] - fExcitation[0,:].imag)) < 1e-12

def test_read_capy_nc_addedMassInterpVals():
    wDes = np.arange(0.1, 2.8, 0.01)
    addedMassInterp, dampingInterp, fExcitationInterp = fdo.read_capy_nc(capyTestFile, wDes=wDes)
...
def test_read_capy_nc_fExInterpRealVals():
    ...
    assert max(abs(refFExInterpReal[:,1] - fExcitationInterp[0,:].real)) < 1e-12
...
def test_read_capy_nc_fExInterpImagVals():
    ...
    assert max(abs(refFExInterpImag[:,1] - fExcitationInterp[0,:].imag)) < 1e-12

def test_call_capy_addedMassShape():
    wRange = np.arange(0.1, 2.9, 0.1)
    meshFName = f'./test_data/float.gdf'
    addedMass, damping, fExcitation = fdo.call_capy(meshFName, wRange)
    assert addedMass.shape == (6, 6, 28)
...
def test_call_capy_addedMassVals():
    ...
    addedMass, damping, fEx = fdo.call_capy(meshFName, wRange)
